...
const ContactForm = () => {
    ...
    //Fill form with data from 'current' obj when edit is clicked
    useEffect (() => {
        if(current !== null){
            setContact(current)
        }else{
    ...
    const onChange = (e) => setContact({...contact, [e.target.name]: e.target.value })  //contact: name, email, phone, type

    const onSubmit = (e) => {
        e.preventDefault()
        if(current === null){  //if adding new contact vs. updating a contact
            addContact(contact)
        }else{
